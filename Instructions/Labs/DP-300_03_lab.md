...
1. In this exercise, you will manage access to the database and its objects. Navigate back to SQL Server Management Studio. The first thing you will do is create two users in the AdventureWorks database. 
    ...
    In the Object Explorer, right click on the AdventureWorks database and select **New Query**. In the new query window, copy and paste the following T-SQL into it. Verify that the code has been copied correctly. Click **Execute** to run the query to create the two users.
