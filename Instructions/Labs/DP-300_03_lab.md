...
2. If prompted to upgrade, select skip. Click the toggle switch under Azure Defender for SQL to **ON**. Click on **Storage account**.
...
1. In this exercise, you will manage access to the database and its objects. Navigate back to SQL Server Management Studio. The first thing you will do is create two users in the AdventureWorks database. Open a new query window and copy and paste the following T-SQL into it. Verify that the code has been copied correctly. Click **Execute** to run the query to create the two users.
